...
import PaymentConfirmationFormDynamic from '../../containers/PaymentConfirmationFormDynamic'
...
class ScreenTransactionConfirmationDynamic extends Component {
  render () {
    console.log('ScreenTransactionConfirmationDynamic props=>', this.props)
    const { navigation } = this.props
    const { qrId, billingId, transactionId, amount, merchantName, merchantId, institutionId } = path(['state', 'params'], navigation) || {}
    return (
      <Container>
        <Header>
          <Left>
        ...
        <Content style={{}} contentContainerStyle={{ margin: 10 }}>
          ...
          <View style={{ padding: 10, marginTop: 10, marginBottom: 20, borderColor: '#000', borderWidth: 3, borderRadius: 20 }}>
            ...
            <PaymentConfirmationFormDynamic
              qrId={qrId} billingId={billingId} transactionId={transactionId} amount={amount} merchantName={merchantName} merchantId={merchantId} institutionId={institutionId}
            />
          </View>
        </Content>
      </Container>
...
export default ScreenTransactionConfirmationDynamic
